...
def test_getPrevPage_0():
    assert Scraper.getPrevPage(slashlessGoogle) == googleB1
def test_getNextPage_0():
    assert Scraper.getNextPage(slashlessGoogle) == googleB21

def test_getPrevPage_Negative():
    assert Scraper.getPrevPage(googleBNeg1) == googleB1
def test_getNextPage_Negative():
    assert Scraper.getNextPage(googleBNeg1) == googleB1

def test_getPrevPage_SuperNegative():
    assert Scraper.getPrevPage("http://www.google.com/?b=-1000000") == googleB1
def test_getNextPage_SuperNegative():
    assert Scraper.getNextPage("http://www.google.com/?b=-1000000") == googleB1

def test_getPrevPage_missingSlash():
    assert Scraper.getPrevPage(slashlessGoogle) == googleB1
def test_getNextPage_missingSlash():
    assert Scraper.getNextPage(slashlessGoogle) == "http://www.google.com/?b=21"

def test_getPrevPage_n100():
    assert Scraper.getPrevPage(yAucN100) == yAucN100B1
def test_getNextPage_n100():
    assert Scraper.getNextPage(yAucN100) == yAucN100B101

def test_getPrevPage_n100b101():
    assert Scraper.getPrevPage(yAucN100B101) == yAucN100B1
def test_getPrevPrevPage_n100b101():
    assert Scraper.getPrevPage(Scraper.getPrevPage(yAucN100B101))\
                 == yAucN100B1

def test_extractAucIDs(examplePage1):
    assert True