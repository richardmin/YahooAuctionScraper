from src.scraper import search_result
import pytest
...
def test_getPrevPageURL_0():
    assert search_result.Scraper.getPrevPageURL(slashlessGoogle) == googleB1
def test_getNextPageURL_0():
    assert search_result.Scraper.getNextPageURL(slashlessGoogle) == googleB21

def test_getPrevPageURL_Negative():
    assert search_result.Scraper.getPrevPageURL(googleBNeg1) == googleB1
def test_getNextPageURL_Negative():
    assert search_result.Scraper.getNextPageURL(googleBNeg1) == googleB1

def test_getPrevPageURL_SuperNegative():
    assert search_result.Scraper.getPrevPageURL(
        "http://www.google.com/?b=-1000000") == googleB1
def test_getNextPageURL_SuperNegative():
    assert search_result.Scraper.getNextPageURL(
        "http://www.google.com/?b=-1000000") == googleB1

def test_getPrevPageURL_missingSlash():
    assert search_result.Scraper.getPrevPageURL(slashlessGoogle) == googleB1
def test_getNextPageURL_missingSlash():
    assert search_result.Scraper.getNextPageURL(slashlessGoogle) == "http://www.google.com/?b=21"

def test_getPrevPageURL_n100():
    assert search_result.Scraper.getPrevPageURL(yAucN100) == yAucN100B1
def test_getNextPageURL_n100():
    assert search_result.Scraper.getNextPageURL(yAucN100) == yAucN100B101

def test_getPrevPageURL_n100b101():
    assert search_result.Scraper.getPrevPageURL(yAucN100B101) == yAucN100B1
def test_getPrevPrevPage_n100b101():
    assert search_result.Scraper.getPrevPageURL(search_result.Scraper.getPrevPageURL(yAucN100B101))\
                 == yAucN100B1
...
def test_extractAucLinks(examplePages, examplePage_aucLinks):
    for page, aucLinks in zip(examplePages, examplePage_aucLinks):
        assert search_result.Scraper.extractAucLinks(page) == aucLinks
